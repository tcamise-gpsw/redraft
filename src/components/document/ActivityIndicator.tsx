...
export function ActivityIndicator({ commit }: { commit: CommitInfo | null }) {
  ...
  return (
    <div className="mb-4 flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900/50 px-4 py-3 text-sm text-slate-300">
      <img src={commit.author.avatarUrl} alt={`${commit.author.login} avatar`} className="h-9 w-9 rounded-full border border-slate-700" />
      <div>
        <p>
          Last edited by <span className="font-medium text-slate-100">@{commit.author.login}</span> · {formatRelativeTime(commit.date)}
        </p>
